...
Shader "DotCrossDot/BlackHoleRaymarchingModified2"
{
	Properties
	{
		...
		_AccretionDiskIntensity("Accretion disk Intensity", Float) = 0.5
		_Noise("Accretion disk noise", 2D) = "" {}
		_RaymarchIterations("Raymarch iterations", Range(1, 512)) = 162
		...
		_ScaleMatcapBG ("ScaleMatcapBG", Range(0,100)) = 1.00
	...
	SubShader
	{
		...
		//Because transparent materials sometimes won't do zwriting properly if it's just in one pass, I think because Unity can't figure out depth.
		Pass {
			Zwrite On
			Cull Off
		}		
		Pass
		{
			...
			vec3 getCol( vec3 dir , float2 uvs) {
				...
				vec3 c0 = texCUBE(_SkyCube, dir);
				vec3 c1 = stars(dir);
				return c0.bgr*_CubeIntensity + c1*2.0;
			}
			...
			float4 raymarch(float3 ro, float3 rd) {
				...
				const int maxstep = _RaymarchIterations;
				...
				float4 skyColor2 = float4(fn01(rd.xz * _AcrretionDiskPulses),0) * tex2D(_Noise, rd.xz * _Noise_ST.xy + _Noise_ST.zw);
				for (int i = 0; i < maxstep; ++i) {
					...
					if (sdfResult < epsilon) {
						...
						lightAccumulation += skyColor2 + col * ((float)i/maxstep) *  log(exp(thickness * distanceToSingularity)) * _AccretionDiskIntensity;
					}
				...
				// Sample the skybox.+
				float3 worldNormal = getNormal(previousPos + previousRayDir);
				float3 col3 = float3(0.,0.,0.);
				float3 skyColor = float3(0.,0.,0.);
				float2 matcapUV = 0.5 + _ScaleMatcapBG * mul((float3x3)UNITY_MATRIX_V, worldNormal).xy;
				...
				float3 blendColor = BlendOverlay(clamp(tex2D(_GrabdTexture, matcapUV * _RefractionLevel)* _RefractionIntensity,0.0,1.0) , hue(skyColor * _BGIntensity,_HueShiftBG));
				backGround = lerp(float4(col3,0.0), _BlackHoleColor, blackHoleInfluence);
				// Return background and light.
				return backGround + saturate(lightAccumulation + (float4(blendColor,0.0) * 1.0-blackHoleInfluence));
			}
